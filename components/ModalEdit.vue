    ...
    <template v-slot:default="{ isActive }">
      <v-card prepend-icon="mdi-cart" :title="infoModal.name">
        ...
        <v-card-text class="px-4" style="height: 300px;">
          <v-card-text v-if="infoModal.size">
            <v-row>
              <v-radio-group
              ...
              <div style="height: 40px; display: flex; align-items: center;">
                <v-icon icon="mdi-delete" size="x-large" color="red" @click="handleDelete"></v-icon>
              </div>
            </v-row>
          ...
          <v-card-text v-else>
            <v-form>
                ...
                <v-text-field
                  ...
                  type="number"
                  v-model="price"
                  :label="'Giá: ' + formatCurrency(price)"
                ></v-text-field>
...
const reloadPrice = computed(() => {
  return (quantity.value && price.value) ? (price.value * quantity.value) : price.value
})
...
const showModal = async (item: any) => {
  ...
  if(item.type) {
    ...
    JSON.parse(item.topping).forEach((element : any) => {
      Object.entries(topping.value).forEach((ele:any, idx: number) => {
        if (ele[1].name === element.name) {
          topping.value[idx+1].isChecked = element.isChecked
...
const handleDelete = async () => {
  await deleteDoc(doc(collection(db, "products"), infoModal.value.id))
  closeModal()
}
...
const handleSubmit = async () => {
  ...
  if(infoModal.value.type) {
    item['type'] = infoModal.value.type
  } else {
    item['size'] = infoModal.value.size;
    item['topping'] = JSON.stringify(newTopping)
    item['quantity'] = quantity.value
  }

  // ✨ add a new todo
  await updateDoc(doc(collection(db, "products"), infoModal.value.id), item)
